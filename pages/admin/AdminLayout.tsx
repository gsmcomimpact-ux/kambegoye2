...
import { LayoutDashboard, Users, Wallet, LogOut, Home, Database, Settings, ShoppingBag, Tags, FileText, Image, FileSpreadsheet, Receipt, Briefcase, AlertTriangle, Handshake, Menu, X, UserCog, Shield, Globe, HardDrive } from 'lucide-react';
    ...
    <>
      ...
      <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
        {visibleNavItems.map((item) => (
          ...
          >
            <item.icon className="h-4.5 w-4.5 mr-3 opacity-70" />
            {item.name}
          </Link>
        ))}
      </nav>
  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="md:hidden bg-white dark:bg-gray-800 shadow-sm p-4 flex justify-between items-center z-10 border-b border-gray-200 dark:border-gray-700">
           <div className="flex items-center gap-3">
             <button onClick={() => setIsMobileMenuOpen(true)} className="text-gray-500 hover:text-gray-700 dark:text-gray-300 p-1">
                <Menu className="h-6 w-6" />
             </button>
             <span className="font-bold text-lg text-gray-900 dark:text-white">Administration</span>
           </div>
